...
module.exports = {
  activate: function() {
    atom.commands.add("atom-text-editor","language-qxzasm:assemble",function(event) {
      var src = atom.workspace.getActiveTextEditor().getText();
      var res = assembler.assemble(src,function(msg) {
        atom.confirm({message:"Assemler error",detailedMessage:msg,buttons:["OK"]});
      });
      if (res) {
        atom.workspace.open().then(function(editor){
          editor.setGrammar(atom.grammars.grammarForScopeName("source.qasm_a"));
          editor.setText(res);
        });
      }
    });
